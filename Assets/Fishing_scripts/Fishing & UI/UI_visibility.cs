...
public class UI_visibility : MonoBehaviour
{
    ...
    // Optionally, reference your game manager to check the game state.
    public FishingGameManager gameManager;

    private void Update()
    {
        // Only perform checks when the game state is one in which UI should normally be visible.
        if (gameManager.currentState != GameState.Countdown && gameManager.currentState != GameState.Playing && gameManager.currentState != GameState.GameOver)
        {
